...
extern "C" void Java_org_linphone_core_LinphoneCoreImpl_setCallLogsDatabaseFile( JNIEnv* env, jobject thiz, jlong lc, jstring jpath) {
        const char* path = GetStringUTFChars(env, jpath);
        linphone_core_set_call_logs_database_file((LinphoneCore*)lc, path);
        ReleaseStringUTFChars(env, jpath, path);
}

extern "C" void Java_org_linphone_core_LinphoneCoreImpl_setCallLogsFriend( JNIEnv* env, jobject thiz, jlong lc, jstring jaccount, jstring jdevice) {
        const char* account = GetStringUTFChars(env, jaccount);
        const char* device = GetStringUTFChars(env, jdevice);
        linphone_core_set_call_logs_friend((LinphoneCore*)lc, account, device);
        ReleaseStringUTFChars(env, jaccount, account);
        ReleaseStringUTFChars(env, jdevice, device);
}
