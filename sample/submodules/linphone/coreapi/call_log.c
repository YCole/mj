...
#if 1
void beauty_mirror_call_log_storage(LinphoneCore *lc, LinphoneCallLog *log) {
	int ret;
	const char *errmsg;
	sqlite3 *db;

	ret=_linphone_sqlite3_open(lc->logs_db_path, &db);
	if(ret != SQLITE_OK) {
		errmsg = sqlite3_errmsg(db);
		ms_error("Error in the opening: %s.\n", errmsg);
		sqlite3_close(db);
		...
		from = linphone_address_as_string(log->from);
		to = linphone_address_as_string(log->to);
		buf = sqlite3_mprintf("INSERT INTO calllogs VALUES(%lld,%Q,%Q,%lld,%i,%i,%i);",
					(int64_t)log->start_date_time,
					strlen(lc->account) == 0 ? (log->dir == LinphoneCallOutgoing ? to : from) : lc->account,
					lc->device,
					(int64_t)time(NULL),
					log->duration,
					log->status == LinphoneCallMissed ? 2 : log->dir,
					0);
		ret=linphone_sql_request_generic(db, buf);
		sqlite3_free(buf);
		sqlite3_close(db);
	}
}
