...
static void linphone_core_register_default_codecs(LinphoneCore *lc){
	...
	linphone_core_register_payload_type(lc,&payload_type_pcmu8000,NULL,FALSE);
	linphone_core_register_payload_type(lc,&payload_type_pcma8000,NULL,FALSE);
	...
	linphone_core_register_payload_type(lc,&payload_type_vp8,NULL,FALSE);
	linphone_core_register_payload_type(lc,&payload_type_h264,"profile-level-id=42801F",TRUE);
	linphone_core_register_payload_type(lc,&payload_type_h265,NULL,FALSE);
...
void linphone_core_set_call_logs_database_file(LinphoneCore *lc, const char *path) {
	if (lc->logs_db_file){
...
void linphone_core_set_call_logs_database_path(LinphoneCore *lc, const char *path) {
	if (lc->logs_db_path){
		ms_free(lc->logs_db_path);
		lc->logs_db_path = NULL;
	}
	if (path) {
		lc->logs_db_path = ms_strdup(path);
	}
}

void linphone_core_set_call_logs_friend(LinphoneCore *lc, const char *account, const char *device) {
	if (lc->account){
		ms_free(lc->account);
		lc->account = NULL;
	}
	if (account) {
		lc->account = ms_strdup(account);
	}
