...
public class CallToast extends Toast {
     ...
     * Toast单例
     */
    private static CallToast toast;
    ...
    private static void initToast(Context context, CharSequence text) {
        try {
            cancelToast();
            toast = new CallToast(context);

            View toastView =LayoutInflater.from(context).inflate(R.layout.call_toast_layout, null);
            LinearLayout relativeLayout = (LinearLayout)toastView.findViewById(R.id.toast_linear);
            RelativeLayout.LayoutParams layoutParams = new RelativeLayout.LayoutParams((int)dpToPx(context, 190), (int)dpToPx(context, 40));
          //  layoutParams.setMargins(85,0,85,0);
            relativeLayout.setLayoutParams(layoutParams);
            TextView toast_text = (TextView) toastView.findViewById(R.id.toast_text);
            toast_text.setText(text);
            toast.setView(toastView);
            toast.setGravity(Gravity.BOTTOM|Gravity.CENTER_VERTICAL, 0, (int)dpToPx(context,215));
        } catch (Exception e) {
            e.printStackTrace();
        }
    }
