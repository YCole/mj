...
//            ExifInterface exif = new ExifInterface();
        ...
        try {
            ...
            if (cursor.moveToNext()) {
                int ori = cursor.getInt(cursor.getColumnIndex(ImageColumns.ORIENTATION));
                return (ori < 0) ? 0 : ori;
            }
        	...
        	Log.e(LOGTAG, "SecurityException: " + uri, e2);
        } catch (Exception e3){
        	Log.e(LOGTAG, "Exception: " + uri, e3);
        } finally {
     ...
     */
    public static Bitmap getConstrainedBitmap(Uri uri, Context context, int maxSideLength, Rect originalBounds) {
        ...
        try {
            ...
            if (bp == null) {
                ...
                Log.w(LOGTAG, "Filepath: " + Filepath);
                if (DrmAdapter.isProtected(Filepath)) {
                    Log.w(LOGTAG, "is DRM");
                    BitmapFactory.Options options = new BitmapFactory.Options();
                    options.inPreferredConfig = Bitmap.Config.ARGB_8888;

                    int type = MediaItem.TYPE_FULLIMAGE;
                    int targetSize = MediaItem.getTargetSize(type);

                    Bitmap bitmap = DrmAdapter.requestDrmDecode(Filepath, options, targetSize, type);
                    if (null != bitmap)
                        originalBounds.set(0, 0, bitmap.getWidth(), bitmap.getHeight());
                    return bitmap;
                }
            }
            // add end
            ...
            if (imageSide > maxSideLength) {
                int shifts = 1 + Integer.numberOfLeadingZeros(maxSideLength) - Integer.numberOfLeadingZeros(imageSide);
                options.inSampleSize <<= shifts;
            }

            // Make sure sample size is reasonable
            if (options.inSampleSize <= 0 || 0 >= (int) (Math.min(w, h) / options.inSampleSize)) {
                return null;
            }
