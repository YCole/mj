...
public class DecodeUtils {
    ...
    public static Bitmap decode(ThreadPool.JobContext jc, FileDescriptor fd,
                                Options options) {
        if (options == null)
            options = new Options();
    ...
    public static Bitmap decodeFaceThumbnail(ThreadPool.JobContext jc, String filePath,
                                             int targetSize, Rect faceRect) {
        ...
        try {
            ...
            jc.setCancelListener(new DecodeCanceller(options));
            options.inJustDecodeBounds = false;
            Bitmap src = BitmapFactory.decodeFileDescriptor(fd, null, options);
            if (jc.isCancelled())
                return null;
            Bitmap faceBitmap = Bitmap.createBitmap(src, faceRect.left, faceRect.top, 
         ...
         // We need to resize down if the decoder does not support inSampleSize
            // (For example, GIF images)
                ...
                if (scale <= 0.5)
                	faceBitmap = BitmapUtils.resizeBitmapByScale(faceBitmap, scale, true);
            return ensureGLCompatibleBitmap(faceBitmap);
        } catch (Exception ex) {
            Log.w(TAG, ex);
            return null;
        } finally {
    ...
    public static Bitmap decodeThumbnail(ThreadPool.JobContext jc, String filePath,
    ...
    public static Bitmap decodeThumbnail(ThreadPool.JobContext jc, FileDescriptor fd,
                                         Options options, int targetSize, int type) {
        if (options == null)
            options = new Options();
        jc.setCancelListener(new DecodeCanceller(options));
        ...
        if (type == MediaItem.TYPE_MICROTHUMBNAIL) {
            ...
            float scale = (float) targetSize / (w + h) * 2;
          /*  options.inSampleSize = BitmapUtils.
                    computeSampleSizeLarger(scale);*/
          ...
          /*  if ((w / options.inSampleSize) * (h / options.inSampleSize) > MAX_PIXEL_COUNT) {
                options.inSampleSize = BitmapUtils.computeSampleSize((float) Math
...
//            options.inSampleSize = BitmapUtils.computeSampleSizeLarger(scale);
        ...
        // We need to resize down if the decoder does not support inSampleSize
        // (For example, GIF images)
        if (type != MediaItem.TYPE_FULLIMAGE) {
            float scale = (float) targetSize
    ...
    public static BitmapRegionDecoder createBitmapRegionDecoder(ThreadPool.JobContext jc,
    ...
    public static Bitmap decodeUsingPool(ThreadPool.JobContext jc,
    ...
    private static Bitmap findCachedBitmap(ThreadPool.JobContext jc, byte[] data,
    ...
    private static Bitmap findCachedBitmap(ThreadPool.JobContext jc,
            FileDescriptor fileDescriptor, Options options) {
        decodeBounds(jc, fileDescriptor, options);
        return GalleryBitmapPool.getInstance().get(options.outWidth,
                options.outHeight);
    }
    public static Options requestOptionFromStream(InputStream stream,
            Options options, int targetSize, int type) {
        if (options == null) {
            options = new Options();
        }
    ...
    public static Bitmap requestDecode(ThreadPool.JobContext jc, InputStream stream,
                                       Options options, int targetSize, int type) {
        ...
        if (result == null) {//the result maybe null
			return null;
		}
     // We need to resize down if the decoder does not support inSampleSize
        // (For example, GIF images)
    ...
    public static Bitmap requestDecode(InputStream stream,
            Options options, int targetSize, int type) {
