...
public abstract class CropDrawingUtils {
    ...
    public static void drawAngleLine(Canvas canvas, RectF bounds) {
        Paint p = new Paint();
        p.setStyle(Paint.Style.STROKE);
        p.setColor(Color.WHITE);
        p.setStrokeWidth(6);
        float stepX = bounds.width();
//        float stepY = bounds.height();
        float leftUpX = bounds.left - 3;
        float leftToUpX = bounds.left - 6;
        float rightUpX = bounds.right + 3;
        float rightToUpX = bounds.right + 6;
        
        float topUpY = bounds.top - 3;
        float topToUpY = bounds.top - 6;
        float bottomDownY = bounds.bottom + 3;
        float bottomToDownY = bounds.bottom + 6;
        
       
            canvas.drawLine(leftUpX, topToUpY, leftUpX, topToUpY + stepX/9, p);
            canvas.drawLine(leftUpX, bottomToDownY, leftUpX, bottomToDownY - stepX/9, p);
            canvas.drawLine(rightUpX, topToUpY, rightUpX, topToUpY + stepX/9, p);
            canvas.drawLine(rightUpX, bottomToDownY, rightUpX, bottomToDownY - stepX/9, p);
     
            canvas.drawLine(leftToUpX, topUpY, leftToUpX + stepX/9 , topUpY, p);
            canvas.drawLine(leftToUpX, bottomDownY, leftToUpX + stepX/9, bottomDownY, p);
            canvas.drawLine(rightToUpX, topUpY, rightToUpX - stepX/9, topUpY, p);
            canvas.drawLine(rightToUpX, bottomDownY, rightToUpX - stepX/9, bottomDownY, p);
         
    }
    
    public static void drawRule(Canvas canvas, RectF bounds) {
        Paint p = new Paint();
        p.setStyle(Paint.Style.STROKE);
